...
class StaticObject
{
    ...
    public function composerNormalize(): Container
    {
        ...
        $composerCache = dag()->cacheVolume("composer-normalize-composer-{$phpVersion}");

        return $this->symfonyContainer
            ->withMountedCache('/root/.composer', $composerCache)
    ...
    private function installTool(Container $container, string $tool): Container
    {
        ...
        $vendorCache = dag()->cacheVolume("{$tool}-vendor-{$phpVersion}");

        return $container
            ->withMountedCache("/bundle/tools/{$tool}/vendor", $vendorCache)
    ...
    {
        $phpVersion = $container->envVariable('PHP_VERSION');
        $composerCache = dag()->cacheVolume("cs2pr-composer-{$phpVersion}");
